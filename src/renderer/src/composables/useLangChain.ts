...
export const useLangChain = () => {
  ...
  const _generateResponse = async (messages: BaseMessage[], chatId: string) => {
    const { provider, model } = settings.getModelById(settings.currentSelectedProvider!.id, settings.currentSelectedModel!.id)!;
    ...
    try {
      await client.invoke(messages.filter(m => {
        if (!AIMessage.isInstance(m)) return true;
        const text = Array.isArray(m.content) ? m.content[0]?.text as string : m.content;
        return text && text.trim() !== "";
      }), {
        callbacks: [{
          handleLLMNewToken: (token: string, _idx: NewTokenIndices, _runId: string, _parentRunId?: string, _tags?: string[], fields?: HandleLLMNewTokenCallbackFields) => {
            content[0]!.text += token
            const reasoning_content = (fields?.chunk as ChatGenerationChunk).message.additional_kwargs?.reasoning_content as string
