 ...
 * 主请求 Schema 的基础对象部分
 */
const siliconFlowCreateSpeechRequestBaseSchema = z.object({
  /**
   * 模型名称
   */
  model: z
    .enum(["fnlp/MOSS-TTSD-v0.5", "CosyVoice2-0.5B"])
    .describe(
  ...
  input: z
    ...
    .max(128000)
    .meta({ ifShow: false })
    .describe("对话文本使用说话人标签来指示轮次：[S1] 和 [S2]。"),
  ...
  max_tokens: z
    ...
    .optional()
    .default(2048)
    .describe("要生成的最大 Token 数量。输入 + 输出不超过 32k 个 Token。"),
  ...
  response_format: z
    ...
    .optional()
    .default("mp3")
    .describe("音频输出格式。支持的格式有 mp3、opus、wav、pcm。")
    .meta({ ifShow: false }),
  ...
  speed: z
    ...
    .optional()
    .default(1)
    .describe("生成音频的语速。选择 0.25 到 4.0 之间的值，默认为 1.0。"),
 ...
 * 校验互斥逻辑的函数
 */
const validateRefinements = (data: any, ctx: z.RefinementCtx) => {
  /**
 ...
 * 主请求 Schema
 */
export const siliconFlowCreateSpeechRequestSchema =
